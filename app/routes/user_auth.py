...
async def login(user: UserLogin, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    try:
        validate_email(user.identifier, check_deliverability=False)
        query = select(User).where(User.email == user.identifier)
    except EmailNotValidError:
        query = select(User).where(User.username == user.identifier)
